...
# DOUBLERS. v0.8.0's parser stopped parsing the PASSIVE-VOICE "twice that many …
# are created/put … instead" replacement template (CR 614.1 — a replacement
# effect over the token-creation / counter-placement event). The ACTIVE-voice
# form ("If an effect would create one or more tokens …, it creates twice that
# many … instead" — Doubling Season, Parallel Lives, Primal Vigor) still parses
# to a token_doubling / counter_doubling Effect, but the PASSIVE form ("… twice
# that many … are created/put … instead") now returns the card with the WHOLE
# replacement ability DROPPED. The 5 marquee passive token doublers come back with
# NO doubling (Adrix and Nev → only Ward; Mondrak / Ojer Taq / Elspeth Storm
# Slayer / Exalted Sunborn keep only their other abilities), and the passive
# counter doublers (Loading Zone, Vorinclex's you-side clause, The Earth Crystal,
# Innkeeper's Talent Level 3) lose counter_replace_bonus (an IR-only migrated key
# with no mirror) and — for The Earth Crystal, whose "twice that many +1/+1
# counters are put" the kept COUNTER_DOUBLING_REGEX word mirror also misses —
# counter_doubling. These two card-level passes (the _recover_dropped_gain_life
# precedent — phase dropped the whole ability, so synthesize the static from the
# joined oracle) re-emit the token_doubling / counter_doubling static so the
# migrated lanes read STRUCTURE again. Append-only: a card already carrying the
# category (the active-voice doublers phase still parses) is untouched, so no
# double-fire. CR 614.1 (replacement effects) / 121 (counters) / 122.
#
# The MULTIPLIER frame the passive replacement uses, as a combinator: "twice that
# many" or "<N> times that many" (Ojer Taq's "three times that many") — the count
# word before "times" is irrelevant, so "times that many" subsumes every N. "that
# many plus" (the adder doublers — Hardened Scales-style "that many plus one") is
# an INCREASE for counters too, so the counter pass accepts it; tokens only ever
# multiply, so the token consequent stays on the multiplier forms.
_INCREASE_THAT_MANY = comb.scan(
    comb.alt(
        comb.phrase({"twice"}, {"that"}, {"many"}),
        comb.phrase({"times"}, {"that"}, {"many"}),
        comb.phrase({"that"}, {"many"}, {"plus"}),
    )
)


def _recover_token_doubling(card: Card, oracle: str) -> Card:
    """Append a synthetic ``token_doubling`` static (scope you) for the passive-voice
    token-creation replacement v0.8.0 drops ("If one or more tokens would be created
    under your control, twice that many of those tokens are created instead" — Adrix
    and Nev, Mondrak, Ojer Taq, Elspeth Storm Slayer, Exalted Sunborn). Append-only:
    a card already carrying a token_doubling Effect (the active-voice doublers phase
    still parses) is left untouched. CR 614.1 / 121."""
    if not card.faces:
        return card
    if any(
        e.category == "token_doubling"
        for ab in card.all_abilities()
        for e in ab.effects
    ):
        return card
    ...
    for clause in re.split(r"[.\n]", text):
        low = clause.lower()
        # A passive token-creation REPLACEMENT: "<token(s)> … created … instead"
        # with a count MULTIPLIER. The cheap substring gate keeps the combinator off
        # the ~99% of cards; "created" (past participle) + "instead" is the passive
        # replacement marker (the active "it creates … instead" form already parses
        # and is screened off by the append-only guard above anyway).
        if (
            "token" in low
            and "created" in low
            and "instead" in low
            and _INCREASE_THAT_MANY.run(clause) is not None
        ):
            scope = "you" if ("your control" in low or "you control" in low) else "any"
            synth.append(
                Ability(
                    kind="static",
                    effects=(
...
def _recover_counter_replacement(card: Card, oracle: str) -> Card:
    ...
    would be put on a … you control, twice that many … counters are put on it
    instead" — Loading Zone, The Earth Crystal; "If you would put one or more
    counters …, put twice that many … instead" — Vorinclex's you-clause, Innkeeper's
    Talent). The signals arm fires both counter_doubling and counter_replace_bonus
    off the category. Append-only: a card already carrying a counter_doubling Effect
    (the active-voice doublers phase still parses — Doubling Season, Branching
    Evolution) is left untouched.

    Per-sentence so the OPPONENT-side REDUCER clause is excluded: Vorinclex's second
    sentence ("If an opponent would put … they put HALF that many … instead") is a
    counter-DECREASE on opponents (NOT a member — it shrinks, doesn't grow), screened
    by requiring the INCREASE frame AND no "opponent" / "half that many" in the
    sentence. The active one-shot doubler on a TARGET's existing counters ("double
    the number of +1/+1 counters on it" — Turtle Van) is NOT a replacement (no
    "would … instead"), so the "would" + "instead" requirement excludes it. CR
    614.1 / 122."""
    if not card.faces:
        return card
    if any(
        e.category == "counter_doubling"
        for ab in card.all_abilities()
        for e in ab.effects
    ):
        return card
