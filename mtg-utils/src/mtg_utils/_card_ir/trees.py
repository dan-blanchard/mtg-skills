"""The corrected concept trees a card resolves to — ONE owner of the pipeline
(ADR-0047; the substrate of ADR-0035 / ADR-0038).

A card's ``oracle_id`` resolves to one :class:`ConceptTree` per phase face record —
strict-loaded against the committed mirror schema, overlaid into the concept
vocabulary (with the ADR-0038 recovery stage at the overlay's tail), then run
through the named overlay-correction stage — plus the predefined-token trees the
card creates whose ability text phase left blank, plus (when the bulk record is
threaded) one text-only tree per bulk face phase has no record for (ADR-0038 W2c).
That composition happens HERE and nowhere else: :func:`face_tree` is the per-record
core, :func:`build_trees` the pure per-oid product, :func:`trees_for` the memoized
production resolver, :func:`seed_trees` the testkit's CI-safe warm.

Two products (ADR-0047): this module's **corrected trees** are what every structural
...
``card-data.json`` record per face; each face is a separate tree — NEVER merged into
one (a merged tree would corrupt card-level reads like ``is_type`` / cmc that only
make sense per-face). Readers union per-tree results. An empty tuple covers no
oracle_id, no phase record / schema, and every face drifting from the committed
schema — each degrading the reader to "nothing structured", never to text-guessing
(post-``PHASE_TAG`` new-set cards land here after a routine ``download-mtgjson``
refresh and must yield no signals, not partial text-derived ones).
"""
...
def clear_caches() -> None:
    ...
    for fn in (_phase_record_index, _committed_schema, _known_tokens_index):
...
#   (CR 122.1) via :func:`~mtg_utils._analysis.tree_synthesis.
...
#   :func:`~mtg_utils._analysis.tree_synthesis._arm_known_token_ramp`.
...
#   :func:`~mtg_utils._analysis.tree_synthesis._arm_known_token_explore`.
...
#   :func:`~mtg_utils._analysis.tree_synthesis.
...
def face_tree(rec: dict, schema: MirrorSchema, *, oracle_id: str) -> ConceptTree | None:
    """ONE phase face record as its corrected concept tree: strict-load against the
    committed mirror schema, overlay it (``build_concept_tree``, which runs the
    ADR-0038 recovery stage at its tail), then run the named overlay-correction
    stage (ADR-0035 Stage-3b (b)). ``None`` when the record drifts from the schema
    ...
    if root is None:
        return None
    return apply_overlay_corrections(
        build_concept_tree(root, name=nm, oracle_id=oracle_id)
    )
...
def build_trees(
    ...
    """The per-face corrected ``ConceptTree`` tuple for ``oid`` from EXPLICIT phase
    face records — pure (no caching, no ``_phase_record_index`` /
    ``ensure_card_data`` I/O beyond the always-committed mirror schema fixture and
    the known-tokens index's own lazy ``functools.cache``).

    Per face record, in order: the face's own tree (:func:`face_tree`), then the
    predefined-token trees the record creates whose OWN ability text phase's parse
    left blank (:func:`_known_token_trees`); then, when ``bulk`` is threaded, one
    text-only tree per bulk face phase has no record for (ADR-0038 W2c). Every
    tree — phase-built, token, text-only — carries the overlay corrections, so a
    reader never sees an uncorrected node.

    :func:`trees_for` wraps this with the production oid→records lookup + memo;
    ...
    trees: list[ConceptTree] = []
    for rec in recs:
        tree = face_tree(rec, schema, oracle_id=oid)
        if tree is not None:
            trees.append(tree)
        trees.extend(
            apply_overlay_corrections(t) for t in _known_token_trees(rec, oracle_id=oid)
        )
    if bulk is not None:
        trees.extend(
            apply_overlay_corrections(t)
            for t in _text_only_trees(bulk, tuple(recs), oracle_id=oid)
        )
    return tuple(trees)
