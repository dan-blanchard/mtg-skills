...
0. **Arena players (MTGA-first path):** if the user mentions Arena, Brawl, Historic Brawl, or wildcards — or provides a Moxfield/Arena deck export but no collection file — run `mtga-import --bulk-data <bulk-data-path> --output-dir <working-dir>` (prefixed with `uv run --directory <skill-install-dir>`) **before** asking them to export a Moxfield CSV. The importer reads the user's MTGA `Player.log`, auto-detects its location on macOS/Windows, and writes two files to the working directory: `collection.json` (parse-deck-compatible; use it in place of the CSV in step 1 by **skipping parse-deck entirely** and passing `collection.json` directly to `find-commanders` in step 2), and `wildcards.json` (read by commander-tuner during Step 3 intake — don't need to read it here, but its presence means the user's wildcard budget is already captured). Unresolved arena_ids or a stale bulk-data warning will appear on stderr; surface them to the user if the count is high. Linux users need to pass `--log-path` explicitly.

1. **Parse the collection** — `parse-deck <absolute-path-to-collection.csv>` produces a parsed deck JSON. `parse-deck` already handles Moxfield CSV, Moxfield deck export, Arena, MTGO, and plain text — use it for any collection format the user gives you. Skip this step if you used `mtga-import` in step 0; the importer already emits a parse-deck-compatible `collection.json`.
