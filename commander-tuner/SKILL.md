...
| Find existing combos or near-misses in the deck | `combo-search <deck.json> --hydrated <hydrated.json>` |
...
- `combo-search <parsed-deck-json> [--hydrated <hydrated.json>] [--max-near-misses N] [--output PATH]` — find existing combos and near-misses in the deck via Commander Spellbook. Pass `--hydrated` to resolve Arena display names to canonical Scryfall names before querying the API. Stdout is a compact text report listing combos (labeled `GAME_WINNING` or `VALUE`) and near-misses (with the missing card identified). Full JSON is written to `$TMPDIR/combo-search-<sha>.json` by default.
