...
that's a copy of X" card (`CopyTokenOf`) emits `token_copy_makers`, **not**
`clone_makers` ("a permanent *becomes* a copy", `BecomeCopy`), even though a clone deck
...
token-copy gear (Helm of the Host, populate) through `clone_makers`'s existing
`_COPY_EXTRA` SubAvenue, while those cards themselves stay out of `clone_makers`
membership.
...
them into both `clone_makers` and `token_copy_makers` membership. Under this decision
that is a false positive on `clone_makers`: those cards do not *become* copies, they
*make* copy tokens. The fix drops the spurious `clone` emission (keeping
`token_copy_makers`); a clone *deck-builder* is unaffected because the want is served by
...
- Cross-archetype reach that *was* implicit in a loose emission must be made explicit as a
  ...
  that's a clone SubAvenue on `token_copy_makers` — a deliberate serve addition, not a
