# deck-forge collections are global, two medium-keyed slots, with derived (not stored) ownership
...
- **Two slots, medium-keyed.** `paper` and `arena`, because the two real libraries are
  ...
  paper → paper) — not by format: a *paper* Historic Brawl correctly reads the paper
  slot, since a paper Historic Brawl is the legal paper "Brawl." (Format-keying via
  `engine.paper_only`, commander → paper / brawl / historic_brawl → arena, was the
  original scheme; it broke once Brawl/Historic Brawl gained a paper/digital **medium**
  toggle, so the slot — and the cost mode, digital → wildcards, paper → USD — moved to
  keying off medium instead.) Reads are **strictly single-slot** — a paper deck never
  consults the Arena slot — and an empty active slot prompts an import, never a silent
  "owns nothing." See the **Medium** glossary term.
...
contaminate ownership across pools that are genuinely different. Medium already cleanly
partitions paper vs. Arena, so the slot auto-picks with zero extra choice at read time.
...
*Amended; original decision revised in place.*
