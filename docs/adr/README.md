...
| 0055 | Limited is a pool-bounded Format family: sealed / draft build from an opened pool | `sealed` / `draft` (40 minimum, no copy limit, no sideboard cap, no legality key); the `pool` zone hydrated by the one seam; legality = `check_pool_containment`; the hub's sideboard is DERIVED (pool less deck) and the pool is the copy limit; Find / Tune search the pool through `card_search.filter_records`; the 17-of-40 land band; `set-scan`, `pool-colors`, the pool panel, the one-click seed |
| 0056 | Tests take real cards by name from the snapshot; they never type card data | `mtg_utils.testkit` (`test_card` / `test_card_ir` / `test_signals` / `snapshot_records`) is the only source of a real card's data in a test; per-printing facts (rarity, set, `games`, prices, ownership) are overlaid as the test's variable; synthetic records only for machinery, with fictional names; `build-card-snapshot` picks up new literal names |
| 0057 | The local Arena card database decides wildcard rarity; MTGJSON is the fallback | `mtg_utils.arena_card_db` finds `Raw_CardDatabase_*.mtga` (`$MTG_SKILLS_ARENA_CARD_DB`, then `Player.log`'s download folder, then standard installs) and reads the lowest rarity among a card's primary printings; `CardPool.rarity_index` projects through it; the fallback counts J21/JMP reprints like any printing; `tests/conftest.py` disables discovery |
| 0058 | Ownership is a medium rule with one owner | `Format.coverage` / `copies_short` (beside `cost_mode`): basic lands free — on Arena always, in paper unless a deck entry asks for a special printing (`is_special_basic_request`), which only that exact printing covers; Arena's four-owned-covers-any-quantity rule; otherwise you have what you own. Printing-level ownership in the neutral `ownership` module. `price-check`, the tuner, deck-forge's served `copies_short` / `covered_by` / `owned` and `lgs-search` all read it |
