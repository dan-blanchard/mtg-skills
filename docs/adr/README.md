...
| 0053 | The hub commits in one place, and Commander discovery owns its cache | `app._commit` is the persist → snapshot → broadcast tail, once; `engine.switch_build` + the lane functions finish ADR-0013; `_deck_forge/discovery.py` (`discover_commanders` / `warm`) with one locked `DiscoveryCache`, served sets keyed by collection content — no invalidation step |
