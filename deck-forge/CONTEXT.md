...
**Busy meter**:
The hub's one long job, reported as it runs: the first-launch card-signal index build (`_analysis/signals_index`, a one-time ~2-4 min `extract_signals` pass over the whole bulk, cached as a sidecar). The hub warms it in a background thread at launch; `state.busy` carries `{job, label, done, total, eta_s}` from the build's own progress hook, every snapshot includes it, and each step is pushed over SSE so every open tab shows the meter. A Find with a theme preset or a discovery run that needs the index meanwhile waits on that same build (one seed lock), never a second one. Null when idle.
