...
- **`art_fetcher.py`** — `fetch-art` CLI. Populates the attributed catalog at `$MTG_SKILLS_CACHE_DIR/attributed-art/` from asciiart.eu: pulls every MTG subtype from Scryfall's catalog endpoints, mines a candidate pool from asciiart.eu category pages (7-day on-disk cache, fail-loud on HTTP errors), scores by target 20×10 / max 30×13, falls back to asciiart.eu `/search?q=` if the category pool misses, and writes attributed `.txt` files with the 3-line license header `proxy_print` parses. A `SKIP_SUBTYPES` set deliberately excludes MTG-only mechanics (Saga, Treasure, …) and plane / setting names (Innistrad, Ravnica, …); a `SYNONYMS` map provides per-subtype fallback queries (`ape → gorilla, monkey`).
