...
- **`export_deck.py`** — Export parsed deck JSON to import text (`N CardName` lines) with sideboard and companion sections. `--style auto|moxfield|arena` (default `auto`): Arena formats (per `FORMAT_CONFIGS[fmt]["arena_format"]`) get Arena's `Commander` / `Companion` / `Deck` / `Sideboard` section headers, which Arena's importer needs to zone the commander and Moxfield also reads; paper formats get bare Moxfield lines.
