...
- **`download_bulk.py`** — Legacy Scryfall bulk downloader with 24h freshness check (retained as a graceful fallback; the Scryfall HTTP API remains the per-card fallback per ADR-0005).
...
- **`deck_signals.py`** — `deck-signals` CLI: the deck's signal lanes (what the commander's oracle text cares about), via `_deck_forge.signals.rank_deck_signals` + `spec_for`. Deterministic; reused by deck-wizard.
- **`slot_budgets.py`** — `slot-budgets` CLI: role-density bands (lands/ramp/card_draw/interaction/board_wipe vs the Command-Zone template), via `_deck_forge.budgets.slot_budgets`.
...
- **`theme_presets.py`** — Registry of named matchers for common MTG mechanics (keyword list + oracle-text regex). Each preset ships with `should_match` / `should_not_match` fixtures pinned in `tests/mtg-utils/test_theme_presets.py`. Used by archetype detection in deck-wizard and cube-wizard. Its structural-view `signal_keys` arm reads `_signal_keys_for`'s per-`oracle_id` memo (`_SIGNAL_KEY_INDEX`), which a whole-pool caller (`card_search`'s `--preset` scan, deck-forge's commander-discovery novelty sweep) can bulk-seed via `seed_signal_key_index(bulk_path)` from the persisted signals-index sidecar (task #90, `_deck_forge/signals_index.py`) instead of paying a live `extract_signals_hybrid` call per card; a no-bulk/no-sidecar deployment falls back to the original per-card live compute unchanged.
...
- **`_phase.py`** — Phase-rs subprocess wrapper. Manages the cached phase
  ...
  SUBSTRATE's data source for predefined-token ability text phase's own
  Token effect nodes don't carry; unlike `ensure_card_data` it NEVER raises
  (`None` on any failure — a pure enhancement, never a hard dependency).
...
Shares `mtg_utils` via symlink to `mtg-utils/src`. A **collaborative, visual** deckbuilder for the Commander family (commander / brawl / historic_brawl, paper + Arena): a Claude Code **skill** run in a normal **interactive** session is the reasoning brain; it spawns a local **FastAPI backend** (`mtg_utils.deck_forge_server`, entry `deck-forge`) that hosts the deterministic core + canonical session state and serves a committed **Svelte SPA** (`deck-forge/frontend/dist`). Two surfaces (D13): the user builds in the browser, the session reasons. Backend internals live in `mtg_utils/_deck_forge/` (mirrors `_stores/` / `_custom_format/`): `signals.py` (the signal facade: `extract_signals_hybrid` is crosswalk-only since ADR-0039 — it resolves each card's per-face concept trees via `_ir_lookup.trees_for`, unions `crosswalk_signals.extract_crosswalk_signals` over them, and runs one merge-level membership floor; `coverage_gate` flags blind spots for the agent), `_ir_lookup.py` (Seam A/B resolvers `trees_for`/`ir_for`; task #92's KNOWN-TOKENS SUBSTRATE lives here — `_known_token_trees` joins a card's `metadata.related_token_ids` to phase's `known-tokens.toml` data (`_phase.ensure_known_tokens`, with a committed offline fallback at `mtg_utils/data/known_tokens_fallback.toml`) and appends one extra zero-unit text-only `ConceptTree` per matched PREDEFINED token whose own Token effect node phase parsed no ability body for, gated to an adjudicated allowlist — task #92 wired `Mutagen` + `Young Hero`; task #95's full-sweep re-adjudication widened it to `Powerstone` / `Gold` (ramp), `Map` (explore_makers), `Junk` (impulse_top_play), and `Wicked` (lifeloss_makers, opponents); task #np_roles closed out the sweep with `Cursed` (the new single_target_neutralize lane), `Royal` (protection_grant), `Sorcerer` / `Shard` (topdeck_selection), `Wizard` (counter_control), `Contract` (lifeloss_makers, opponents), and `Spellgorger Weird` (spellcast_matters) — so every existing "does any tree do X" lane picks up the token's real ability for free), `crosswalk_signals.py` (the Layer-3 signal lanes over the ADR-0035 concept overlay — typed structural reads, `PORTED_KEYS` is the served-key set; the surviving text reads are the gap-gated, self-retiring bridges ledgered in `bridge_ledger.py`), `_subtypes.py` (baked creature-subtype vocabulary harvested from bulk — the precision gate that keeps captured subjects real), `signals_index.py` (task #90: `oracle_id -> "key|scope|subject"` idents persisted for the WHOLE bulk pool at `<bulk_path>.signals.pkl`, so a cold whole-pool preset/novelty scan pays the ~2-4 min `extract_signals_hybrid` pass once instead of once per process; invalidated by a content hash over every source file the AST-audited import closure of `signals.py` reaches, `_phase.PHASE_TAG`, and the bulk file's own identity — never a hand-bumped version constant; `build-signals-index` prebuilds it), `signal_specs.py` (signal → serve/search specs, scope-discriminating, with per-subject dynamic specs for tribal signals), `budgets.py` (slot budgets vs the soft Command Zone template), `ranking.py` (transparent multi-axis scoring — synergy/curve/price, never EDHREC popularity), `agent_bridge.py` (browser↔session long-poll queue), `events.py` (SSE hub), `persistence.py` (autosave + build library), `exporters.py`, `app.py` (the FastAPI factory `build_app(state)`), `production.py` (`default_state()` — real bulk or graceful no-bulk), `state.py` (`DeckSession` + the injectable `ForgeState`), and `phase_crosscheck.py` (read-only audit harness, entry `deck-forge-phase-crosscheck`: projects phase-rs's parsed `card-data.json` to normalized mechanic tags and diffs them against detector firings, joined by `oracle_id`, via an editable `CROSSWALK` mapping payoff lanes→phase triggers / doer lanes→phase effects; a deterministic second opinion for finding detector gaps/over-fires, never ground truth — phase's parse is incomplete and crowd-independent). **Load-bearing contract (ADR-0009): the session-agent never names a card from memory — it proposes patterns/searches/judgments; the deterministic core (`card_search` + `theme_presets` + Commander Spellbook) names real cards.** Billing-safe by being an interactive skill, never Agent-SDK/ACP/`claude -p` (ADR-0010). Autosave/resume departs from ADR-0003 (ADR-0011). See `deck-forge/CONTEXT.md` for vocabulary (signal / synergy package / avenue / candidate / slot budget / curve gate / no-listing card). The deterministic core also runs agent-less (search/curve/combos/budgets/finalize) for non-Claude-Code users.
...
**Real-card test fixtures (ADR-0027 / task #25 / ADR-0039).** Signal tests evaluate the SAME Card IR real cards parse into — not a hand-built `_ir(Ability(...))` shape that drifts from production. `mtg_utils.testkit` serves `test_card(name)` (minimal Scryfall record), `test_card_ir(name)` (the crosswalk-era compat `Card`, built on demand from the snapshot's stored raw phase face records — the same shape `ir_for` serves in production), and `test_signals(name)` (the production `extract_signals_hybrid`, with the concept-tree memo pre-seeded from those records) from a committed snapshot at `tests/fixtures/card_snapshot.json` (schema 2) — so real crosswalk trees run in CI with **no** sidecar/bulk/phase/network. The snapshot is usage-derived: `build-card-snapshot` AST-scans the tests for `test_card`/`test_card_ir` usages — direct literals, `@pytest.mark.parametrize` columns that feed such a call through a bare variable, and `_REAL_CASES` name-table values — resolves each name to its gameplay printing, stores its minimal Scryfall record plus its raw phase face records (the INPUT, never a baked IR), and self-validates that the minimal record loses no signal vs the full bulk record. It carries the `crosswalk_sidecar_version` and `phase_tag` it was captured at; loading asserts both match, so a mirror-schema / compat / phase bump fails loudly until the snapshot is regenerated. The card-data source is MTGJSON (ADR-0033): `build-card-snapshot` sources the minimal records from the MTGJSON-backed `bulk_loader`; the stored phase records are phase's own parse either way. The flagship consumer is `tests/deck-forge/test_migrated_keys.py` (every migrated key proven against real IR via `_REAL_CASES`; a few `_SYNTHETIC_CASES` stay hand-built for placeholder mechanics / known recovery gaps).
