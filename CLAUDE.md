...
- **`formats.py`** — The format module (ADR-0045): one frozen `Format` value per format (`FORMATS[name]`, `get_format`, `Format.for_deck(deck_json)`) that owns legality (`legality` / `is_legal` / `commander_eligibility`, with the Competitive Brawl override and the Arena-pool gate folded in), medium (`media` / `resolve_medium` / `cost_mode`), size (`size_choices` / `resolve_deck_size` / `size_rule`), and the SPA table (`format_options`). `HydratedDeck.format` returns it. Ground truth for the "Supported Deck Formats" table below; no caller re-derives a format fact from the table.
