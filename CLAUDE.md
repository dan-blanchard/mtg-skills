...
- **`art_fetcher.py`** — `fetch-art` CLI. Populates the attributed catalog at `$MTG_SKILLS_CACHE_DIR/attributed-art/` from two sources: asciiart.eu (hardcoded curated category list, optional `/search?q=` fallback) and Christopher Johnson's collection at asciiart.website (categories auto-discovered from `browse.php`, ~635 of them; each cat.php page carries JSON-LD metadata and inline `<pre data-artwork-id=N>` art bodies that we zip by ID). Pulls every MTG subtype from Scryfall's catalog endpoints, scores candidates by target 20×10 / max 30×13, 7-day on-disk cache, fail-loud on HTTP errors. Each written file's 3-line header points at the per-source attribution terms — `_parse_cards` returns dicts tagged `_source: "eu"` or `"website"` and `write_art` formats the header accordingly. A `SKIP_SUBTYPES` set deliberately excludes MTG-only mechanics (Saga, Treasure, …) and plane / setting names (Innistrad, Ravnica, …); a `SYNONYMS` map provides per-subtype fallback queries (`ape → gorilla, monkey`).
...
Shares `mtg_utils` via symlink to `mtg-utils/src`. Renders printable PDF proxies from a parsed deck JSON: `proxy-print --kind cards` emits one proxy per copy of every card in the deck; `proxy-print --kind tokens` emits one proxy per distinct token kind the deck produces (deduped by Scryfall `oracle_id`, sources tracked for the footer). Both modes share one render template — name banner / ASCII art / type banner / oracle text / P/T — auto-fitting body proportions based on how much oracle text the card has. Two-tier ASCII art: a hand-curated **local catalog** at `mtg-utils/src/mtg_utils/data/card_art/*.txt` (one file per card subtype with card-type and ultimate-generic fallbacks) plus an optional **attributed catalog** at `$MTG_SKILLS_CACHE_DIR/attributed-art/` whose files carry a license header and propagate an `art by <Name>` credit to the proxy footer. Lookup is per-slug interleaved (`attributed/<sub>` → `local/<sub>` → next slug), then card-type, then `local/_generic.txt`. The attributed catalog ships empty; populate it with `fetch-art`, which mines asciiart.eu for one candidate per Scryfall subtype. Art is P/T-independent — every Soldier shares `soldier.txt`. See `proxy-printer/CONTEXT.md` for the catalog / lookup chain / signature / artist credit vocabulary, and `docs/adr/0006-attributed-art-catalog.md` for the ship-empty + per-slug interleaving rationale. The skill is callable standalone or by deck-wizard / cube-wizard at the end of a build session; `parse-deck` runs upstream to produce the deck JSON.
